...
const GENRES = [
  { id: "genre-action", name: "Action / Aventure", icon: "fa-solid fa-hand-fist" },
  { id: "genre-rpg", name: "RPG", icon: "fa-solid fa-dragon" },
  { id: "genre-fps", name: "FPS / Shooter", icon: "fa-solid fa-crosshairs" },
  { id: "genre-strategie", name: "Stratégie / Gestion", icon: "fa-solid fa-chess-knight" },
  { id: "genre-sport", name: "Sport / Course", icon: "fa-solid fa-flag-checkered" },
  { id: "genre-autre", name: "Autre (Général)", icon: "fa-solid fa-layer-group" },
];
      ...
      {!searchTerm &&
          GENRES.map((genre) => (
              ...
              >
                ...
                <span style={{ color: "#c4b5fd", fontSize: "0.9rem", fontWeight: "bold" }}>
                  Général : {genre.name}
                </span>
              </div>
          ))}
...
const CreateThreadModal = ({ user, onClose, onCreated }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      setErrorMsg("Veuillez remplir le titre, le contenu ET lier un jeu ou genre !");
      return;
    }
    setSubmitting(true);
    try {
      ...
      <>
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <h3 className="section-title" style={{ margin: 0, border: "none", display: "flex", alignItems: "center", gap: "8px" }}>
              <i className="fa-solid fa-pen-to-square" style={{ color: "currentColor" }}></i>
              Nouveau sujet
            </h3>
            <button
                onClick={onClose}
          ...
          <textarea
              ...
              style={{ width: "100%", height: "120px", marginBottom: "15px", boxSizing: "border-box", resize: "vertical" }}
          />
          <label className="game-genre" style={{ display: "block", marginBottom: "8px", fontSize: "0.9rem" }}>
            <i className="fa-solid fa-link" style={{ marginRight: "6px" }}></i> Jeu ou Genre lié *
          </label>
                ...
                >
                  {newThread.gameCoverId ? (
                      <img
                          ...
                          alt=""
                          style={{ width: "24px", height: "30px", borderRadius: "4px", objectFit: "cover" }}
                      />
                  ) : (
                      <i className={getIconForGenre(newThread.gameId)} style={{ fontSize: "1.2rem", color: "#c084fc", width: "24px", textAlign: "center" }}></i>
                  )}
                  <span style={{ flex: 1, fontWeight: "600" }}>{newThread.gameName}</span>
                  <button
                      style={{
                        background: "none",
                        border: "none",
          ...
          {view === "list" ? (
              <>
                <div className="section-header">
                  ...
                  >
                    <i className="fa-solid fa-plus" style={{ marginRight: "6px" }}></i> Nouveau sujet
                  </button>
                </div>
                <div className="comments-list-modern">
                  {loading ? (
                      ...
                      <p style={{ color: "#94a3b8", textAlign: "center", padding: "40px 0" }}>
                        <i className="fa-regular fa-comments" style={{ fontSize: "2rem", marginBottom: "10px", display: "block" }}></i>
                          ...
                          >
                            <div style={{ flexShrink: 0 }}>
                              {thread.gameCoverId ? (
                                  ...
                                  >
                                    <i className={String(thread.gameId).startsWith("genre") ? getIconForGenre(thread.gameId) : "fa-solid fa-gamepad"} style={{ color: "#c4b5fd" }}></i>
                                  </div>
                              )}
                            </div>
                            <div style={{ flex: 1 }}>
                              ...
                              >
                                <i className={String(thread.gameId).startsWith("genre") ? getIconForGenre(thread.gameId) : "fa-solid fa-gamepad"}></i>
                                {thread.gameName}
                              </div>
                            </div>
                            <span
              ...
              <div className="game-card-modern" style={{ padding: "20px" }}>
                <button className="category-btn" onClick={() => setView("list")}>
                  <i className="fa-solid fa-arrow-left" style={{ marginRight: "6px" }}></i> Retour
                </button>
                <div
                    style={{
                      display: "flex",
                ...
                >
                  ...
                  Par {selectedThread.authorName}
                    ...
                    {selectedThread.gameName && (
                        ...
                        >
                          <i className={String(selectedThread.gameId).startsWith("genre") ? getIconForGenre(selectedThread.gameId) : "fa-solid fa-gamepad"}></i>
                          {selectedThread.gameName}
                        </div>
                    )}
                  </div>
                ...
                <div className="comments-list-modern">
                  {posts.map((post) => (
                      <div key={post.id} className="post-item" style={{ padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                        <div
                            style={{
                              display: "flex",
                              gap: "12px",
                        ...
                        >
                          ...
                          />
                          <div>
                            <div style={{ fontWeight: "600", color: "#c084fc", fontSize: "0.9rem", marginBottom: "4px" }}>
                              {post.authorName}
                            </div>
                            <p style={{ margin: 0, fontSize: "0.95rem", color: "inherit", lineHeight: "1.5" }}>{post.content}</p>
                          </div>
                        </div>
                      </div>
                  ))}
                ...
                <textarea
                    className="filter-select"
                    style={{ width: "100%", height: "80px", marginBottom: "10px", boxSizing: "border-box", resize: "vertical" }}
                    placeholder="Écrire une réponse..."
                    value={newPostContent}
                    onChange={(e) => setNewPostContent(e.target.value)}
                    required
                />
                      <button type="submit" className="nav-user-btn" style={{ justifyContent: "center", width: "100%" }} disabled={!newPostContent.trim()}>
                        <i className="fa-solid fa-reply" style={{ marginRight: "6px" }}></i> Répondre
                      </button>
                    </form>
                ) : (
                    <p style={{ textAlign: "center", color: "#64748b", marginTop: "20px" }}>
                      <i className="fa-solid fa-lock" style={{marginRight: "6px"}}></i> Connectez-vous pour répondre.
                    </p>
                )}
              </div>
          )}
