...
exports.followUser = async (req, res, next) => {
  try {
    ...
    if (followDoc.exists) {
      return res
        .status(409)
        .json({ success: false, msg: "Vous suivez déjà cet utilisateur" });
    }
    ...
    res
      .status(201)
      .json({ success: true, msg: "Vous suivez maintenant cet utilisateur" });
  } catch (error) {
    next(error);
  }
};
...
exports.unfollowUser = async (req, res, next) => {
  try {
    ...
    if (!followDoc.exists) {
      return res
        .status(404)
        .json({ success: false, msg: "Vous ne suivez pas cet utilisateur" });
    }
