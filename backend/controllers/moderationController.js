...
exports.reportContent = async (req, res, next) => {
    try {
        ...
        if (!targetId || !reason) {
            return res.status(400).json({ success: false, msg: 'ID cible et raison requis' });
        }
        ...
        res.status(201).json({ success: true, msg: 'Signalement envoyé aux administrateurs' });
    } catch (error) {
        next(error);
    }
};
...
exports.adminDeleteReview = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ success: true, msg: 'Critique supprimée par modération' });
    } catch (error) {
        next(error);
    }
};
...
exports.adminHighlightReview = async (req, res, next) => {
    try {
        ...
        await Logger.log('admin_highlighted_review', req.user.id, { reviewId, isHighlighted });

        res.status(200).json({ success: true, msg: isHighlighted ? 'Critique mise en avant' : 'Mise en avant retirée', isHighlighted });
    } catch (error) {
        next(error);
    }
};