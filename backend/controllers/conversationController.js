...
exports.getOrCreateConversation = async (req, res, next) => {
    try {
        ...
        if (!isMutual) {
            return res.status(403).json({ success: false, msg: 'Vous devez vous suivre mutuellement pour échanger des messages' });
        }
        ...
        if (convDoc.exists) {
            return res.json({ success: true, created: false, conversation: { id: convDoc.id, ...convDoc.data() } });
        }
...
exports.sendMessage = async (req, res, next) => {
    try {
        ...
        if (!text.trim() && attachments.length === 0) {
            return res.status(400).json({ success: false, msg: 'Un message ne peut pas être vide' });
        }
...
exports.markAsRead = async (req, res, next) => {
    try {
        ...
        if (!messageDoc.exists) {
            return res.status(404).json({ success: false, msg: 'Message introuvable' });
        }
