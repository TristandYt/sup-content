...
process.env.GCLOUD_PROJECT =
  process.env.FIREBASE_PROJECT_ID || "sup-content-tristan";
...
const buildApp = () => {
  ...
  app.use(
    ...
    authMiddleware,
    ensureFirestoreProfile,
    require("../Routes/followRouter"),
  );
  app.use(
    "/api/feeds",
    authMiddleware,
    ensureFirestoreProfile,
    require("../Routes/feedRouter"),
  );
  app.use(
    "/api/conversations",
    authMiddleware,
    ensureFirestoreProfile,
    require("../Routes/conversationRouter"),
  );

  app.get(
    "/api/users/me/export",
    authMiddleware,
    require("../controllers/userController").exportUserData,
  );
  app.post(
    "/api/interactions/reviews/:reviewId/like",
    authMiddleware,
    require("../controllers/reviewController").toggleLikeReview,
  );
  app.post(
    "/api/interactions/reviews/:reviewId/comments",
    authMiddleware,
    require("../controllers/reviewController").commentReview,
  );
  app.get(
    "/api/notifications",
    authMiddleware,
    require("../controllers/notificationController").getMyNotifications,
  );
  app.get(
    "/api/notifications/stream",
    authMiddleware,
    require("../controllers/notificationController").streamNotifications,
  );
  app.get(
    "/api/moderation/reports",
    authMiddleware,
    isAdmin,
    require("../controllers/moderationController").getReports,
  );
  app.patch(
    "/api/notifications/:notificationId/read",
    authMiddleware,
    require("../controllers/notificationController").markAsRead,
  );
  app.post(
    "/api/moderation/report",
    authMiddleware,
    require("../controllers/moderationController").reportContent,
  );
  app.delete(
    "/api/moderation/reviews/:reviewId",
    authMiddleware,
    isAdmin,
    require("../controllers/moderationController").adminDeleteReview,
  );
  app.patch(
    "/api/moderation/reviews/:reviewId/highlight",
    authMiddleware,
    isAdmin,
    require("../controllers/moderationController").adminHighlightReview,
  );
  ...
  app.get(
    "/api/recommendations",
    authMiddleware,
    require("../controllers/searchController").getRecommendations,
  );

  app.post(
    "/api/lists/custom",
    authMiddleware,
    require("../controllers/listController").createList,
  );
  app.get(
    "/api/lists/custom/me",
    authMiddleware,
    require("../controllers/listController").getMyLists,
  );
  app.get(
    "/api/lists/custom/:listId",
    authMiddleware,
    require("../controllers/listController").getListDetails,
  );
  app.post(
    "/api/lists/custom/:listId/games",
    authMiddleware,
    require("../controllers/listController").addGameToList,
  );

  app.delete(
    "/api/users/favorites/:gameId",
    authMiddleware,
    require("../controllers/userController").removeFavorite,
  );
  app.delete(
    "/api/users/account",
    authMiddleware,
  ...
  app.get(
    ...
    authMiddleware,
    isAdmin,
    require("../controllers/userController").getLogs,
  );
  app.post(
    "/api/users/promote/:userId",
    authMiddleware,
    isAdmin,
    require("../controllers/userController").promoteUser,
  );
  app.post(
    "/api/moderation/users/:userId/ban",
    authMiddleware,
    isAdmin,
    require("../controllers/moderationController").adminBanUser,
  );
  app.get(
    "/api/users/me/stats",
    authMiddleware,
    require("../controllers/listController").getMyLibraryStats,
  );
...
describe("Connexions OAuth2 (Google, GitHub, Facebook)", () => {
  it("200 — crée automatiquement le profil Firestore pour une connexion Google", async () => {
    const res = await request(app)
      ...
      .set({ Authorization: "Bearer oauth-google" });
  ...
  it("200 — crée automatiquement le profil Firestore pour une connexion GitHub", async () => {
    const res = await request(app)
      .get("/api/users/profile")
      .set({ Authorization: "Bearer oauth-github" });
    expect(res.status).toBe(200);
  ...
  it("200 — crée automatiquement le profil Firestore pour une connexion Facebook", async () => {
    const res = await request(app)
      .get("/api/users/profile")
      .set({ Authorization: "Bearer oauth-facebook" });
    expect(res.status).toBe(200);
...
describe("GET /api/games/popular", () => {
  ...
  it("200 — accepte les paramètres sortBy et order", async () => {
    const res = await request(app).get(
      "/api/games/popular?sortBy=total_rating&order=asc",
    );
    expect(res.status).toBe(200);
  });
});
...
describe("GET /api/games/filtered", () => {
  it("200 — retourne une liste de jeux filtrés par style, genre, plateforme", async () => {
    const res = await request(app)
      .get("/api/games/filtered?genre=12&platform=6&style=3")
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
  });
...
describe("PUT /api/users/profile", () => {
  ...
  it("200 — met à jour le profil avec l'avatar (PP) et le site web", async () => {
    ...
    expect(doc.data().profileData.avatarUrl).toBe(
...
describe("Administration (Users)", () => {
  ...
  it("200 — POST /api/users/promote/:userId : promeut un utilisateur", async () => {
    const res = await request(app)
      .post("/api/users/promote/user_bob_999")
      .set(AUTH_HEADER)
      .send({ role: "admin" });
    expect(res.status).toBe(200);
  });
...
describe("DELETE /api/users/account", () => {
  it("200 — supprime le compte", async () => {
    ...
    await db
      .collection("users")
      .doc("user_delete_me")
      .set({ username: "delete_me" });

    const res = await request(app)
      .delete("/api/users/account")
      .set({ Authorization: "Bearer delete-token" });
    expect(res.status).toBe(200);
...
describe("GET /api/lists/library", () => {
  ...
  it("200 — accepte le filtre ?status=playing", async () => {
    const res = await request(app)
      .get("/api/lists/library?status=playing")
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
  });
  ...
  it("400 — rejette un statut de filtre invalide", async () => {
    const res = await request(app)
      .get("/api/lists/library?status=invalid")
      .set(AUTH_HEADER);
    expect(res.status).toBe(400);
  });
});
...
describe("Forum (Sujets et Réponses)", () => {
  ...
  it("201 — ajoute une réponse à un sujet avec l'avatar de l'utilisateur", async () => {
    if (!threadId)
      throw new Error("Thread non créé lors de l'étape précédente");
    const res = await request(app)
  ...
  it("200 — récupère les sujets filtrés par jeu (gameId)", async () => {
    const res = await request(app)
      .get("/api/forum/threads?gameId=1020")
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  ...
  it("404 — rejette la lecture si le sujet n'existe pas", async () => {
    const res = await request(app)
      .get("/api/forum/threads/ghost_thread")
      .set(AUTH_HEADER);
    expect(res.status).toBe(404);
  });
...
describe("DELETE /api/lists/library/:gameId", () => {
  it("200 — supprime un jeu de la bibliothèque", async () => {
    ...
    await request(app)
      .post("/api/lists/status")
      .set(AUTH_HEADER)
      .send({ gameId: "9999", status: "dropped" });
...
describe("GET /api/reviews/me", () => {
  ...
  it("200 — retourne les reviews avec pagination (limit)", async () => {
    const res = await request(app)
      .get("/api/reviews/me?page=1&limit=2")
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
...
describe("DELETE /api/reviews/:gameId", () => {
  it("200 — supprime une review existante", async () => {
    ...
    await request(app)
      .post("/api/reviews")
      .set(AUTH_HEADER)
      .send({ gameId: "8888", rating: 3, text: "bof" });

    const res = await request(app).delete("/api/reviews/8888").set(AUTH_HEADER);
    expect(res.status).toBe(200);
  });

  it("404 — rejette si review inexistante", async () => {
    const res = await request(app).delete("/api/reviews/9999").set(AUTH_HEADER);
    expect(res.status).toBe(404);
  });
});
...
describe("Interactions Sociales", () => {
  ...
  it("404 — like : rejette si la critique n'existe pas", async () => {
    const res = await request(app)
      .post("/api/interactions/reviews/ghost_review/like")
      .set(AUTH_HEADER);
    expect(res.status).toBe(404);
  });
});
...
describe("POST /api/follows/:userId", () => {
  ...
  it("201 — permet de suivre un nouvel utilisateur", async () => {
    await db
      .collection("users")
      .doc("user_charlie_003")
      .set({ username: "charlie" });
    const res = await request(app)
      .post("/api/follows/user_charlie_003")
      .set(AUTH_HEADER);
    expect(res.status).toBe(201);
  });
});
...
describe("DELETE /api/follows/:userId", () => {
  it("200 — permet de se désabonner", async () => {
    const res = await request(app)
      .delete("/api/follows/user_charlie_003")
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
  });
...
describe("GET /api/feeds", () => {
  it("200 — retourne le feed", async () => {
    ...
    const hasLibraryUpdate = res.body.feed.some(
      (item) => item.type === "LIBRARY_UPDATED",
    );
...
describe("Notifications", () => {
  it("200 — récupère la liste des notifications", async () => {
    ...
    const hasReco = res.body.notifications.some(
  ...
  it("200 — marque une notification comme lue", async () => {
    const notifRef = await db
      ...
      .add({ userId: "user_alice_001", isRead: false });
    const res = await request(app)
      .patch(`/api/notifications/${notifRef.id}/read`)
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
  });
  ...
  it("404 — marquer lue : rejette si inexistante", async () => {
    const res = await request(app)
      .patch("/api/notifications/ghost_notif/read")
      .set(AUTH_HEADER);
    expect(res.status).toBe(404);
  });
  ...
  it("403 — marquer lue : rejette si n'appartient pas à l'utilisateur", async () => {
    const notifRef = await db
      .collection("notifications")
      .add({ userId: "user_bob_999", isRead: false });
    const res = await request(app)
      .patch(`/api/notifications/${notifRef.id}/read`)
      .set(AUTH_HEADER);
    expect(res.status).toBe(403);
...
describe("Modération", () => {
  ...
  it("200 — admin peut mettre en avant une critique", async () => {
    const res = await request(app)
      .patch("/api/moderation/reviews/user_alice_001_1020/highlight")
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  ...
  it("200 — permet à un admin de supprimer une critique (supposé protégé par middleware)", async () => {
    const res = await request(app)
      .delete("/api/moderation/reviews/user_alice_001_1020")
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  });
...
describe("Listes personnalisées (2.2.2)", () => {
  ...
  it("200 — ajoute un jeu à une liste", async () => {
    ...
    const res = await request(app)
      .post(`/api/lists/custom/${listId}/games`)
      .set(AUTH_HEADER)
      .send({ gameId: "1020", gameName: "Trine" });
    expect(res.status).toBe(200);

    const detailsRes = await request(app)
      .get(`/api/lists/custom/${listId}`)
      .set(AUTH_HEADER);
  ...
  it("403 — rejette l'ajout d'un jeu à une liste qui ne nous appartient pas", async () => {
    ...
    const res = await request(app)
      .post(`/api/lists/custom/${docRef.id}/games`)
      .set(AUTH_HEADER)
      .send({ gameId: "1020" });
    expect(res.status).toBe(403);
  });
  ...
  it("404 — ajoute un jeu : rejette si liste inexistante", async () => {
    const res = await request(app)
      .post("/api/lists/custom/ghost_list/games")
      .set(AUTH_HEADER)
      .send({ gameId: "1020" });
    expect(res.status).toBe(404);
  });
  ...
  it("403 — détails : rejette si la liste est privée et ne nous appartient pas", async () => {
    ...
    const res = await request(app)
      .get(`/api/lists/custom/${docRef.id}`)
      .set(AUTH_HEADER);
    expect(res.status).toBe(403);
  });
  ...
  it("200 — retire un jeu d'une liste personnalisée", async () => {
    ...
    const createRes = await request(app)
      .post("/api/lists/custom")
      .set(AUTH_HEADER)
      .send({ name: "Liste de test suppression" });
    const listId = createRes.body.listId;
    ...
    const res = await request(app)
      .delete(`/api/lists/custom/${listId}/games/1020`)
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    ...
    const detailsRes = await request(app)
      .get(`/api/lists/custom/${listId}`)
      .set(AUTH_HEADER);
    const gameExists = detailsRes.body.list.games.some(
      (g) => g.gameId === "1020",
    );
...
describe("POST /api/conversations/:id/messages", () => {
  ...
  it("201 — envoie un message avec succès", async () => {
    ...
    await db
      .collection("follows")
      .doc("user_bob_999_user_alice_001")
      .set({ followerId: "user_bob_999", followingId: "user_alice_001" });
    const res = await request(app)
      .post(`/api/conversations/${convId}/messages`)
      .set(AUTH_HEADER)
      .send({ text: "Nouveau message" });
    expect(res.status).toBe(201);
  });

  it("404 — rejette si la conversation n'existe pas pour un message", async () => {
    const res = await request(app)
      .post("/api/conversations/ghost_conv/messages")
      .set(AUTH_HEADER)
      .send({ text: "Hello" });
    expect(res.status).toBe(404);
  });
...
describe("GET /api/conversations/:id/messages", () => {
  ...
  it("403 — refuse l'accès aux messages si l'utilisateur n'est pas participant", async () => {
    await db
      .collection("conversations")
      .doc("user_bob_999_user_charlie_003")
      .set({ participants: ["user_bob_999", "user_charlie_003"] });
    const res = await request(app)
...
describe("PATCH /api/conversations/.../read", () => {
  it("200 — marque un message comme lu", async () => {
    ...
    await db
      .collection("conversations")
      .doc(convId)
      .set({ participants: ["user_alice_001", "user_bob_999"] });
  ...
  it("404 — rejette si la conversation n'existe pas (lecture)", async () => {
    const res = await request(app)
      .patch("/api/conversations/ghost_conv/messages/msg123/read")
      .set(AUTH_HEADER);
    expect(res.status).toBe(404);
  });
  ...
  it("404 — rejette si le message n'existe pas (lecture)", async () => {
    const convId = "user_alice_001_user_bob_999";
    const res = await request(app)
      .patch(`/api/conversations/${convId}/messages/ghost_msg/read`)
      .set(AUTH_HEADER);
    expect(res.status).toBe(404);
  });
});
...
describe("Listes personnalisées (Update & Delete)", () => {
  ...
  it("403 — PUT /api/lists/custom/:listId : rejette si ce n'est pas notre liste", async () => {
    const docRef = await db
      ...
      .add({ userId: "user_bob_999", name: "Bob List", isPrivate: true });
    const res = await request(app)
      .put(`/api/lists/custom/${docRef.id}`)
      .set(AUTH_HEADER)
      .send({ name: "Hacked" });
    expect(res.status).toBe(403);
  });
  ...
  it("200 — DELETE /api/lists/custom/:listId : supprime la liste", async () => {
    const res = await request(app)
      .delete(`/api/lists/custom/${listId}`)
      .set(AUTH_HEADER);
    expect(res.status).toBe(200);
  });
});
